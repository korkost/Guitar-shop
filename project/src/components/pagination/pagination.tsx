...
function Pagination(): JSX.Element {
  ...
  const prevPath = generatePath(`${AppRoute.Catalog}/page_:number`, {
    number: `${prevBtnValue}`,
  });
  const nextPath = generatePath(`${AppRoute.Catalog}/page_number`, {
    number: `${nextBtnValue}`,
  });
    ...
    <div className="pagination page-content__pagination">
      <ul className="pagination__list">
        ...
        {paginationLinks.map((linkNumber) => {
          const path = generatePath(`${AppRoute.Catalog}/page_:number`, {
            number: `${linkNumber}`,
          });
          ...
          return (
            <li
              className={`pagination__page ${
                currentPageNumber === linkNumber && "pagination__page--active"
              }`}
              key={linkNumber}
            >
              <Link
